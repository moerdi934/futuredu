...
// Updated SelectCustomField component with improved dropdown positioning and styling
...
export const SelectCustomField: React.FC<SelectCustomProps> = ({
  ...
  // Super high z-index for dropdown positioning
  const getZIndex = (isOpen: boolean) => {
    if (!isOpen) return 1;
    return 99999; // Very high z-index to ensure it's above everything
  };

  // Custom styles with improved dropdown positioning and high z-index
  const customSelectStylesImproved = {
    control: (provided: any, state: any) => ({
      ...provided,
      backgroundColor: 'rgba(255, 255, 255, 0.95)',
      backdropFilter: 'blur(4px)',
    ...
    menuList: (provided: any) => ({
      ...provided,
      padding: '8px',
      // Scrollable with max height for 6 items (approximately 60px per item + padding)
      maxHeight: '380px', // 6 items * ~60px + padding
      overflowY: 'auto',
    ...
    <Form.Group className="mb-3" ref={selectRef}>
      ...
      >
        <Select
          ...
          styles={customSelectStylesImproved}
          // Don't use portal to maintain positioning control
          menuPortalTarget={null}
          // Better positioning and behavior
          menuShouldScrollIntoView={false}
  ...
  // Perbaikan untuk YesNoField component di FormComponentLayout.tsx
  export const YesNoField: React.FC<YesNoProps> = ({ 
    ...
    // Card variant (responsive version)
    return (
      <Form.Group className="mb-3">
