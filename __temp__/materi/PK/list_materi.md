# Pengetahuan Kuantitatif (PK) - UTBK
## Kurikulum Terstruktur untuk Siswa SMA

---

## SECTION 1: Aljabar dan Persamaan

### Topic 1.1: Operasi Aljabar Dasar

**Materi 1.1.1: Pengenalan Aljabar dan Sifat-sifat Operasi** *(GRATIS)*
* Konsep variabel, konstanta, dan koefisien dalam aljabar
* Sifat komutatif, asosiatif, dan distributif pada operasi bilangan
* Operasi penjumlahan, pengurangan, perkalian, dan pembagian bentuk aljabar
* Penyederhanaan bentuk aljabar sederhana
* Penggunaan tanda kurung dalam ekspresi aljabar

**Materi 1.1.2: Operasi Pecahan Aljabar**
* Konsep pecahan aljabar dan syarat penyebutnya
* Penyederhanaan pecahan aljabar dengan faktorisasi
* Penjumlahan dan pengurangan pecahan aljabar
* Perkalian dan pembagian pecahan aljabar
* Penyelesaian soal aplikasi pecahan aljabar

**Materi 1.1.3: Eksponen dan Bentuk Akar**
* Sifat-sifat bilangan berpangkat (eksponen)
* Operasi pada bilangan berpangkat
* Bentuk akar dan sifat-sifatnya
* Merasionalkan bentuk akar
* Operasi pada bentuk akar (penjumlahan, pengurangan, perkalian, pembagian)

**Materi 1.1.4: Faktorisasi Aljabar**
* Faktor persekutuan terbesar (FPB) dalam aljabar
* Faktorisasi bentuk ax + ay
* Faktorisasi selisih dua kuadrat (a² - b²)
* Faktorisasi trinomial kuadrat sempurna
* Faktorisasi bentuk ax² + bx + c

**Materi 1.1.5: Operasi Aljabar Lanjutan**
* Perpangkatan bentuk aljabar (a + b)²
* Perkalian bentuk aljabar khusus
* Pembagian bentuk aljabar dengan bentuk aljabar
* Teorema sisa dan teorema faktor
* Horner untuk pembagian polinomial

### Topic 1.2: Persamaan Linear

**Materi 1.2.1: Pengenalan Persamaan Linear Satu Variabel** *(GRATIS)*
* Konsep persamaan dan penyelesaian persamaan
* Bentuk umum persamaan linear satu variabel (PLSV)
* Prinsip kesetaraan dalam persamaan
* Langkah-langkah menyelesaikan PLSV
* Aplikasi PLSV dalam soal cerita sederhana

**Materi 1.2.2: Pertidaksamaan Linear Satu Variabel**
* Konsep pertidaksamaan dan notasi (<, >, ≤, ≥)
* Sifat-sifat pertidaksamaan
* Menyelesaikan pertidaksamaan linear satu variabel
* Representasi himpunan penyelesaian pada garis bilangan
* Aplikasi pertidaksamaan dalam masalah sehari-hari

**Materi 1.2.3: Sistem Persamaan Linear Dua Variabel**
* Konsep sistem persamaan linear dua variabel (SPLDV)
* Metode grafik untuk menyelesaikan SPLDV
* Metode substitusi untuk menyelesaikan SPLDV
* Metode eliminasi untuk menyelesaikan SPLDV
* Jenis-jenis penyelesaian SPLDV (tunggal, tak hingga, tidak ada)

**Materi 1.2.4: Aplikasi Sistem Persamaan Linear**
* Menerjemahkan soal cerita ke dalam SPLDV
* Masalah campuran dan harga barang
* Masalah umur dan perbandingan
* Masalah kecepatan dan jarak
* Masalah pekerjaan dan waktu

**Materi 1.2.5: Fungsi Linear dan Grafiknya**
* Konsep fungsi dan notasi fungsi
* Bentuk umum fungsi linear f(x) = ax + b
* Gradien (kemiringan) garis dan interpretasinya
* Menggambar grafik fungsi linear
* Hubungan gradien dengan sudut kemiringan

**Materi 1.2.6: Persamaan Garis Lurus**
* Bentuk-bentuk persamaan garis (y = mx + c, ax + by + c = 0)
* Menentukan persamaan garis melalui dua titik
* Menentukan persamaan garis dengan gradien dan satu titik
* Garis sejajar dan tegak lurus
* Jarak titik ke garis

**Materi 1.2.7: Pertidaksamaan Linear Dua Variabel**
* Konsep pertidaksamaan linear dua variabel
* Menggambar daerah penyelesaian pertidaksamaan
* Sistem pertidaksamaan linear dua variabel
* Menentukan daerah himpunan penyelesaian
* Aplikasi dalam program linear sederhana

### Topic 1.3: Persamaan Kuadrat

**Materi 1.3.1: Pengenalan Persamaan Kuadrat** *(GRATIS)*
* Bentuk umum persamaan kuadrat ax² + bx + c = 0
* Pengertian akar-akar persamaan kuadrat
* Cara menentukan akar dengan pemfaktoran
* Konsep akar kembar dan akar berlainan
* Hubungan koefisien dengan grafik parabola

**Materi 1.3.2: Menyelesaikan Persamaan Kuadrat dengan Melengkapkan Kuadrat**
* Konsep kuadrat sempurna
* Teknik melengkapkan kuadrat
* Langkah-langkah menyelesaikan persamaan kuadrat
* Menentukan nilai maksimum atau minimum
* Aplikasi dalam masalah optimasi sederhana

**Materi 1.3.3: Rumus ABC (Kuadratik)**
* Penurunan rumus ABC dari melengkapkan kuadrat
* Penggunaan rumus ABC untuk menyelesaikan persamaan kuadrat
* Diskriminan (D = b² - 4ac) dan jenis akar
* Syarat akar real, rasional, dan irasional
* Efisiensi penggunaan rumus ABC

**Materi 1.3.4: Sifat-sifat Akar Persamaan Kuadrat**
* Jumlah akar-akar (x₁ + x₂ = -b/a)
* Hasil kali akar-akar (x₁ × x₂ = c/a)
* Menyusun persamaan kuadrat baru dari akar yang diketahui
* Hubungan akar dengan koefisien
* Aplikasi rumus Vieta dalam soal

**Materi 1.3.5: Fungsi Kuadrat**
* Bentuk umum fungsi kuadrat f(x) = ax² + bx + c
* Grafik fungsi kuadrat (parabola)
* Titik puncak (vertex) parabola
* Sumbu simetri parabola
* Nilai maksimum dan minimum fungsi kuadrat

**Materi 1.3.6: Grafik dan Karakteristik Parabola**
* Pengaruh koefisien a terhadap bentuk parabola
* Pengaruh koefisien b dan c terhadap posisi parabola
* Titik potong dengan sumbu x dan y
* Menggambar sketsa grafik parabola
* Aplikasi parabola dalam masalah nyata

**Materi 1.3.7: Pertidaksamaan Kuadrat**
* Konsep pertidaksamaan kuadrat
* Menyelesaikan pertidaksamaan kuadrat dengan garis bilangan
* Menentukan daerah penyelesaian
* Hubungan dengan grafik parabola
* Aplikasi pertidaksamaan kuadrat

### Topic 1.4: Fungsi Eksponen dan Logaritma

**Materi 1.4.1: Pengenalan Fungsi Eksponen** *(GRATIS)*
* Konsep bilangan berpangkat dan fungsi eksponen
* Bentuk umum fungsi eksponen f(x) = aˣ
* Grafik fungsi eksponen
* Sifat-sifat fungsi eksponen (monoton naik/turun)
* Aplikasi fungsi eksponen dalam pertumbuhan dan peluruhan

**Materi 1.4.2: Persamaan Eksponen**
* Persamaan eksponen bentuk aˣ = aʸ
* Persamaan eksponen dengan substitusi
* Persamaan eksponen bentuk a^(f(x)) = a^(g(x))
* Persamaan eksponen bentuk a^(f(x)) = b
* Teknik penyelesaian persamaan eksponen kompleks

**Materi 1.4.3: Pertidaksamaan Eksponen**
* Konsep pertidaksamaan eksponen
* Sifat pertidaksamaan untuk a > 1 dan 0 < a < 1
* Menyelesaikan pertidaksamaan eksponen sederhana
* Pertidaksamaan eksponen dengan substitusi
* Aplikasi pertidaksamaan eksponen

**Materi 1.4.4: Pengenalan Logaritma**
* Definisi logaritma sebagai invers eksponen
* Hubungan aˣ = b dan ᵃlog b = x
* Sifat-sifat dasar logaritma
* Logaritma basis 10 dan logaritma natural (ln)
* Perubahan basis logaritma

**Materi 1.4.5: Operasi dan Sifat Logaritma**
* Sifat logaritma perkalian (ᵃlog (bc) = ᵃlog b + ᵃlog c)
* Sifat logaritma pembagian (ᵃlog (b/c) = ᵃlog b - ᵃlog c)
* Sifat logaritma perpangkatan (ᵃlog bⁿ = n · ᵃlog b)
* Rumus perubahan basis
* Logaritma dari 1 dan logaritma dari basis

**Materi 1.4.6: Persamaan Logaritma**
* Persamaan logaritma bentuk ᵃlog f(x) = ᵃlog g(x)
* Persamaan logaritma dengan satu logaritma
* Persamaan logaritma bentuk ᵃlog f(x) = b
* Persamaan logaritma dengan substitusi
* Syarat-syarat penyelesaian persamaan logaritma

**Materi 1.4.7: Pertidaksamaan Logaritma dan Fungsi Logaritma**
* Konsep pertidaksamaan logaritma
* Sifat pertidaksamaan untuk a > 1 dan 0 < a < 1
* Menyelesaikan pertidaksamaan logaritma
* Grafik fungsi logaritma f(x) = ᵃlog x
* Aplikasi logaritma dalam skala (pH, Richter, desibel)

---

## SECTION 2: Aritmatika

### Topic 2.1: Sistem Bilangan

**Materi 2.1.1: Pengenalan Sistem Bilangan** *(GRATIS)*
* Jenis-jenis bilangan (asli, cacah, bulat, rasional, irasional, real)
* Diagram himpunan bilangan
* Garis bilangan dan representasi bilangan
* Hubungan antar himpunan bilangan
* Contoh bilangan dari setiap jenis

**Materi 2.1.2: Sifat-sifat Bilangan Bulat**
* Bilangan genap dan ganjil
* Bilangan positif, negatif, dan nol
* Operasi pada bilangan bulat
* Sifat tertutup, komutatif, asosiatif pada bilangan bulat
* Aplikasi bilangan bulat dalam kehidupan sehari-hari

**Materi 2.1.3: Bilangan Rasional dan Irasional**
* Definisi bilangan rasional (dapat dinyatakan sebagai p/q)
* Contoh bilangan rasional dan operasinya
* Definisi bilangan irasional (tidak dapat dinyatakan sebagai pecahan)
* Contoh bilangan irasional (√2, π, e)
* Membuktikan suatu bilangan rasional atau irasional

**Materi 2.1.4: Operasi pada Bilangan Pecahan**
* Pecahan biasa, campuran, dan desimal
* Penjumlahan dan pengurangan pecahan
* Perkalian dan pembagian pecahan
* Mengubah bentuk pecahan (biasa, campuran, desimal, persen)
* Aplikasi pecahan dalam masalah sehari-hari

**Materi 2.1.5: Bilangan Berpangkat**
* Pengertian bilangan berpangkat
* Sifat-sifat bilangan berpangkat bulat positif
* Bilangan berpangkat nol dan negatif
* Notasi ilmiah (scientific notation)
* Operasi pada bilangan berpangkat

**Materi 2.1.6: Akar dan Operasinya**
* Pengertian akar kuadrat dan akar pangkat n
* Sifat-sifat akar
* Operasi penjumlahan dan pengurangan bentuk akar
* Perkalian dan pembagian bentuk akar
* Merasionalkan penyebut bentuk akar

### Topic 2.2: Bilangan Prima dan Faktorisasi

**Materi 2.2.1: Pengenalan Bilangan Prima** *(GRATIS)*
* Definisi bilangan prima
* Cara menentukan bilangan prima
* Bilangan komposit dan perbedaannya dengan prima
* Tabel bilangan prima dari 1-100
* Penerapan bilangan prima dalam kehidupan

**Materi 2.2.2: Faktorisasi Prima**
* Konsep faktor dan kelipatan
* Pohon faktor untuk faktorisasi prima
* Menuliskan bilangan dalam bentuk faktorisasi prima
* Faktorisasi prima dengan pembagian berulang
* Aplikasi faktorisasi prima

**Materi 2.2.3: FPB dan KPK**
* Pengertian Faktor Persekutuan Terbesar (FPB)
* Pengertian Kelipatan Persekutuan Terkecil (KPK)
* Menentukan FPB dengan faktorisasi prima
* Menentukan KPK dengan faktorisasi prima
* Hubungan FPB dan KPK (FPB × KPK = a × b)

**Materi 2.2.4: Aplikasi FPB dan KPK**
* Soal cerita tentang pembagian rata
* Soal cerita tentang kejadian berulang
* Masalah pengaturan dan pengelompokan
* Masalah pola dan siklus
* Kombinasi masalah FPB dan KPK

**Materi 2.2.5: Keterbagian Bilangan**
* Aturan keterbagian oleh 2, 3, 4, 5
* Aturan keterbagian oleh 6, 8, 9, 10
* Aturan keterbagian oleh 11
* Aplikasi aturan keterbagian
* Menentukan sisa pembagian

**Materi 2.2.6: Bilangan Bulat dalam Operasi Modulo**
* Konsep modulo (sisa pembagian)
* Operasi modulo dasar
* Sifat-sifat operasi modulo
* Aplikasi modulo dalam pola bilangan
* Teorema sisa dalam konteks modulo

### Topic 2.3: Barisan dan Deret

**Materi 2.3.1: Pengenalan Barisan dan Deret** *(GRATIS)*
* Pengertian barisan bilangan
* Pengertian deret sebagai jumlah barisan
* Notasi barisan (U₁, U₂, U₃, ..., Uₙ)
* Suku ke-n dan beda/rasio
* Perbedaan barisan aritmatika dan geometri

**Materi 2.3.2: Barisan Aritmatika**
* Definisi barisan aritmatika
* Beda (b) dalam barisan aritmatika
* Rumus suku ke-n: Uₙ = a + (n-1)b
* Menentukan suku tengah barisan aritmatika
* Sisipan dalam barisan aritmatika

**Materi 2.3.3: Deret Aritmatika**
* Definisi deret aritmatika
* Rumus jumlah n suku pertama: Sₙ = n/2(2a + (n-1)b)
* Rumus alternatif: Sₙ = n/2(a + Uₙ)
* Hubungan Uₙ dan Sₙ
* Aplikasi deret aritmatika dalam masalah sehari-hari

**Materi 2.3.4: Barisan Geometri**
* Definisi barisan geometri
* Rasio (r) dalam barisan geometri
* Rumus suku ke-n: Uₙ = arⁿ⁻¹
* Suku tengah barisan geometri
* Sisipan dalam barisan geometri

**Materi 2.3.5: Deret Geometri**
* Definisi deret geometri
* Rumus jumlah n suku pertama: Sₙ = a(rⁿ - 1)/(r - 1) untuk r ≠ 1
* Rumus alternatif: Sₙ = a(1 - rⁿ)/(1 - r)
* Hubungan Uₙ dan Sₙ dalam barisan geometri
* Aplikasi deret geometri

**Materi 2.3.6: Deret Geometri Tak Hingga**
* Konsep deret geometri tak hingga
* Syarat konvergen: |r| < 1
* Rumus jumlah deret geometri tak hingga: S∞ = a/(1-r)
* Mengubah pecahan desimal berulang ke pecahan biasa
* Aplikasi deret geometri tak hingga

**Materi 2.3.7: Pola Bilangan dan Aplikasi Barisan Deret**
* Pola bilangan segitiga, persegi, persegi panjang
* Barisan Fibonacci
* Pola bilangan lainnya
* Aplikasi dalam pertumbuhan dan penurunan
* Soal cerita kombinasi barisan dan deret

### Topic 2.4: Aritmatika Sosial

**Materi 2.4.1: Pengenalan Aritmatika Sosial** *(GRATIS)*
* Konsep harga pembelian, penjualan, dan modal
* Pengertian untung dan rugi
* Persentase untung dan rugi
* Menghitung harga beli dan harga jual
* Aplikasi dalam perdagangan sederhana

**Materi 2.4.2: Rabat (Diskon), Bruto, Neto, dan Tara**
* Pengertian rabat/diskon dan cara menghitungnya
* Harga setelah diskon
* Diskon bertingkat
* Pengertian bruto, neto, dan tara
* Hubungan bruto, neto, dan tara

**Materi 2.4.3: Pajak dan Bunga Tunggal**
* Pengertian pajak dan jenis-jenisnya (PPN, PPh)
* Menghitung pajak
* Harga sebelum dan sesudah pajak
* Pengertian bunga tunggal
* Rumus bunga tunggal: B = (M × p × t)/100

**Materi 2.4.4: Bunga Majemuk dan Anuitas**
* Pengertian bunga majemuk
* Rumus bunga majemuk: M = M₀(1 + i)ⁿ
* Perbedaan bunga tunggal dan majemuk
* Pengertian anuitas
* Aplikasi bunga majemuk dalam investasi

**Materi 2.4.5: Persentase dan Aplikasinya**
* Konsep persentase
* Mengubah pecahan dan desimal ke persentase
* Menghitung persentase dari suatu bilangan
* Persentase kenaikan dan penurunan
* Aplikasi persentase dalam kehidupan sehari-hari

**Materi 2.4.6: Perbandingan dan Skala**
* Pengertian perbandingan (rasio)
* Perbandingan senilai
* Perbandingan berbalik nilai
* Pengertian skala pada peta
* Aplikasi perbandingan dalam masalah sehari-hari

---

## SECTION 3: Geometri

### Topic 3.1: Geometri Datar

**Materi 3.1.1: Pengenalan Bangun Datar** *(GRATIS)*
* Jenis-jenis bangun datar (segitiga, segiempat, lingkaran)
* Unsur-unsur bangun datar
* Keliling dan luas konsep dasar
* Satuan panjang dan luas
* Aplikasi bangun datar dalam kehidupan

**Materi 3.1.2: Segitiga**
* Jenis-jenis segitiga berdasarkan sisi dan sudut
* Keliling segitiga
* Luas segitiga (½ × alas × tinggi)
* Rumus Heron untuk luas segitiga
* Garis-garis istimewa pada segitiga

**Materi 3.1.3: Segiempat (Persegi, Persegi Panjang, Jajargenjang)**
* Sifat-sifat persegi dan persegi panjang
* Keliling dan luas persegi dan persegi panjang
* Sifat-sifat jajargenjang
* Keliling dan luas jajargenjang
* Hubungan diagonal pada segiempat

**Materi 3.1.4: Segiempat (Trapesium, Belah Ketupat, Layang-layang)**
* Jenis-jenis trapesium
* Keliling dan luas trapesium
* Sifat-sifat belah ketupat
* Keliling dan luas belah ketupat dan layang-layang
* Perbandingan berbagai segiempat

**Materi 3.1.5: Lingkaran**
* Unsur-unsur lingkaran (jari-jari, diameter, tali busur, busur, juring, tembereng)
* Keliling lingkaran: K = 2πr atau πd
* Luas lingkaran: L = πr²
* Hubungan sudut pusat dengan panjang busur
* Luas juring dan tembereng

**Materi 3.1.6: Aplikasi Bangun Datar Gabungan**
* Menghitung keliling bangun gabungan
* Menghitung luas bangun gabungan
* Menentukan bagian yang diarsir
* Soal cerita tentang bangun datar
* Optimasi luas dan keliling

### Topic 3.2: Geometri Ruang

**Materi 3.2.1: Pengenalan Bangun Ruang** *(GRATIS)*
* Jenis-jenis bangun ruang (kubus, balok, prisma, limas, tabung, kerucut, bola)
* Unsur-unsur bangun ruang (titik sudut, rusuk, sisi, diagonal)
* Konsep luas permukaan dan volume
* Satuan volume
* Jaring-jaring bangun ruang

**Materi 3.2.2: Kubus dan Balok**
* Sifat-sifat kubus dan balok
* Luas permukaan kubus dan balok
* Volume kubus dan balok
* Diagonal bidang, diagonal ruang, dan bidang diagonal
* Aplikasi kubus dan balok

**Materi 3.2.3: Prisma dan Limas**
* Pengertian prisma dan jenis-jenisnya
* Luas permukaan dan volume prisma
* Pengertian limas dan jenis-jenisnya
* Luas permukaan dan volume limas
* Hubungan volume prisma dan limas

**Materi 3.2.4: Tabung (Silinder)**
* Unsur-unsur tabung
* Luas permukaan tabung (2πr² + 2πrt)
* Volume tabung (πr²t)
* Tabung tanpa tutup
* Aplikasi tabung dalam kehidupan

**Materi 3.2.5: Kerucut**
* Unsur-unsur kerucut
* Hubungan r, t, dan s (garis pelukis)
* Luas permukaan kerucut (πr² + πrs)
* Volume kerucut (⅓πr²t)
* Aplikasi kerucut

**Materi 3.2.6: Bola**
* Unsur-unsur bola
* Luas permukaan bola (4πr²)
* Volume bola (4/3πr³)
* Belahan bola (hemisphere)
* Aplikasi bola dalam kehidupan

**Materi 3.2.7: Bangun Ruang Gabungan dan Aplikasi**
* Menghitung luas permukaan bangun gabungan
* Menghitung volume bangun gabungan
* Perbandingan volume bangun ruang
* Soal cerita tentang bangun ruang
* Optimasi volume dan luas permukaan

### Topic 3.3: Transformasi

**Materi 3.3.1: Pengenalan Transformasi Geometri** *(GRATIS)*
* Pengertian transformasi geometri
* Jenis-jenis transformasi (translasi, refleksi, rotasi, dilatasi)
* Konsep titik bayangan
* Sifat-sifat yang berubah dan tetap
* Visualisasi transformasi pada koordinat

**Materi 3.3.2: Translasi (Pergeseran)**
* Pengertian translasi
* Notasi translasi dengan vektor
* Menentukan bayangan titik oleh translasi
* Menentukan bayangan bangun datar oleh translasi
* Komposisi translasi

**Materi 3.3.3: Refleksi (Pencerminan)**
* Pengertian refleksi
* Refleksi terhadap sumbu x dan sumbu y
* Refleksi terhadap garis y = x dan y = -x
* Refleksi terhadap titik asal O(0,0)
* Komposisi refleksi

**Materi 3.3.4: Rotasi (Perputaran)**
* Pengertian rotasi
* Rotasi dengan pusat O(0,0) sebesar 90°, 180°, 270°
* Rotasi dengan pusat sembarang
* Arah rotasi (searah/berlawanan jarum jam)
* Komposisi rotasi

**Materi 3.3.5: Dilatasi (Perbesaran/Pengecilan)**
* Pengertian dilatasi
* Faktor skala dilatasi (k > 1, k = 1, 0 < k < 1, k < 0)
* Dilatasi dengan pusat O(0,0)
* Dilatasi dengan pusat sembarang
* Sifat-sifat dilatasi

**Materi 3.3.6: Komposisi Transformasi**
* Komposisi dua transformasi atau lebih
* Urutan transformasi dan pengaruhnya
* Matriks transformasi
* Mencari transformasi tunggal dari komposisi
* Aplikasi transformasi dalam masalah

### Topic 3.4: Trigonometri

**Materi 3.4.1: Pengenalan Trigonometri** *(GRATIS)*
* Konsep perbandingan trigonometri
* Segitiga siku-siku dan unsur-unsurnya
* Definisi sin, cos, tan dalam segitiga siku-siku
* Nilai sin, cos, tan untuk sudut istimewa (30°, 45°, 60°)
* Aplikasi trigonometri dalam kehidupan

**Materi 3.4.2: Perbandingan Trigonometri**
* Perbandingan sin, cos, tan
* Perbandingan csc, sec, cot
* Hubungan antar perbandingan trigonometri
* Nilai perbandingan trigonometri sudut istimewa
* Tabel dan diagram nilai trigonometri

**Materi 3.4.3: Identitas Trigonometri Dasar**
* Identitas Pythagoras: sin²θ + cos²θ = 1
* Identitas tan θ = sin θ / cos θ
* Identitas 1 + tan²θ = sec²θ
* Identitas 1 + cot²θ = csc²θ
* Pembuktian identitas trigonometri sederhana

**Materi 3.4.4: Aturan Sinus dan Cosinus**
* Teorema Pythagoras dalam segitiga siku-siku
* Aturan sinus: a/sin A = b/sin B = c/sin C
* Aturan cosinus: c² = a² + b² - 2ab cos C
* Aplikasi aturan sinus dan cosinus
* Menentukan unsur segitiga sembarang

**Materi 3.4.5: Sudut Elevasi dan Depresi**
* Pengertian sudut elevasi
* Pengertian sudut depresi
* Menghitung tinggi objek dengan sudut elevasi
* Menghitung jarak dengan sudut depresi
* Aplikasi dalam navigasi dan survei

**Materi 3.4.6: Luas Segitiga dengan Trigonometri**
* Rumus luas segitiga: L = ½ab sin C
* Menghitung luas dengan dua sisi dan sudut apit
* Aplikasi dalam berbagai jenis segitiga
* Hubungan dengan rumus Heron
* Soal aplikasi luas dengan trigonometri

**Materi 3.4.7: Grafik Fungsi Trigonometri**
* Grafik fungsi y = sin x
* Grafik fungsi y = cos x
* Grafik fungsi y = tan x
* Amplitudo, periode, dan fase
* Aplikasi fungsi trigonometri

---

## SECTION 4: Statistika dan Peluang

### Topic 4.1: Statistika

**Materi 4.1.1: Pengenalan Statistika dan Pengumpulan Data** *(GRATIS)*
* Pengertian statistika dan data
* Jenis data (kualitatif dan kuantitatif)
* Data tunggal dan data berkelompok
* Cara pengumpulan data
* Populasi dan sampel

**Materi 4.1.2: Penyajian Data**
* Tabel distribusi frekuensi
* Diagram batang
* Diagram garis
* Diagram lingkaran
* Histogram dan poligon frekuensi

**Materi 4.1.3: Ukuran Pemusatan Data (Mean)**
* Pengertian mean (rata-rata)
* Mean data tunggal
* Mean data berkelompok
* Rata-rata tertimbang
* Aplikasi mean dalam kehidupan

**Materi 4.1.4: Ukuran Pemusatan Data (Median dan Modus)**
* Pengertian median
* Median data tunggal dan berkelompok
* Pengertian modus
* Modus data tunggal dan berkelompok
* Hubungan mean, median, dan modus

**Materi 4.1.5: Ukuran Penyebaran Data (Jangkauan dan Kuartil)**
* Pengertian jangkauan (range)
* Kuartil (Q₁, Q₂, Q₃)
* Jangkauan interkuartil
* Menentukan kuartil data tunggal dan berkelompok
* Diagram kotak garis (box plot)

**Materi 4.1.6: Ukuran Penyebaran Data (Variansi dan Simpangan Baku)**
* Pengertian variansi
* Simpangan baku (standar deviasi)
* Rumus variansi dan simpangan baku data tunggal
* Rumus variansi dan simpangan baku data berkelompok
* Interpretasi simpangan baku

**Materi 4.1.7: Aplikasi Statistika**
* Membandingkan data dengan ukuran pemusatan
* Analisis data menggunakan ukuran penyebaran
* Interpretasi grafik dan diagram
* Pengambilan keputusan berdasarkan data
* Soal cerita aplikasi statistika

### Topic 4.2: Peluang

**Materi 4.2.1: Pengenalan Peluang** *(GRATIS)*
* Pengertian percobaan, ruang sampel, dan kejadian
* Titik sampel dan menentukan ruang sampel
* Pengertian peluang suatu kejadian
* Rumus peluang: P(A) = n(A)/n(S)
* Peluang kejadian pasti dan kejadian mustahil

**Materi 4.2.2: Peluang Kejadian Majemuk**
* Kejadian saling lepas (mutually exclusive)
* Peluang kejadian bersatu: P(A∪B)
* Kejadian saling bebas (independent)
* Peluang kejadian majemuk: P(A∩B)
* Komplemen suatu kejadian

**Materi 4.2.3: Peluang Bersyarat**
* Pengertian peluang bersyarat
* Rumus peluang bersyarat: P(A|B)
* Hubungan dengan peluang gabungan
* Diagram pohon untuk peluang bersyarat
* Aplikasi peluang bersyarat

**Materi 4.2.4: Frekuensi Harapan**
* Pengertian frekuensi harapan
* Rumus frekuensi harapan
* Menghitung frekuensi harapan suatu kejadian
* Perbedaan frekuensi harapan dan frekuensi relatif
* Aplikasi frekuensi harapan

**Materi 4.2.5: Permutasi**
* Pengertian permutasi
* Rumus permutasi n! dan nPr
* Permutasi dengan unsur yang sama
* Permutasi siklik (melingkar)
* Aplikasi permutasi dalam masalah

**Materi 4.2.6: Kombinasi**
* Pengertian kombinasi
* Perbedaan permutasi dan kombinasi
* Rumus kombinasi nCr
* Sifat-sifat kombinasi
* Aplikasi kombinasi dalam masalah

**Materi 4.2.7: Aplikasi Permutasi, Kombinasi, dan Peluang**
* Soal gabungan permutasi dan kombinasi
* Menentukan peluang dengan permutasi/kombinasi
* Kaidah pencacahan (aturan perkalian dan penjumlahan)
* Soal cerita kompleks
* Peluang dalam permainan dan lotere

### Topic 4.3: Himpunan

**Materi 4.3.1: Pengenalan Himpunan** *(GRATIS)*
* Pengertian himpunan dan notasinya
* Anggota dan bukan anggota himpunan
* Himpunan kosong, himpunan semesta
* Cara menyatakan himpunan (deskripsi, notasi pembentuk, mendaftar)
* Kardinalitas himpunan

**Materi 4.3.2: Himpunan Bagian**
* Pengertian himpunan bagian (subset)
* Himpunan bagian sejati
* Banyak himpunan bagian: 2ⁿ
* Hubungan antar himpunan
* Kesamaan dua himpunan

**Materi 4.3.3: Operasi Himpunan**
* Irisan (intersection) himpunan
* Gabungan (union) himpunan
* Selisih (difference) himpunan
* Komplemen himpunan
* Sifat-sifat operasi himpunan

**Materi 4.3.4: Diagram Venn**
* Menggambar diagram Venn satu himpunan
* Diagram Venn dua himpunan
* Diagram Venn tiga himpunan
* Daerah yang diarsir dalam diagram Venn
* Menentukan kardinalitas dengan diagram Venn

**Materi 4.3.5: Aplikasi Himpunan**
* Soal cerita tentang survei
* Masalah dua himpunan
* Masalah tiga himpunan
* Rumus n(A∪B) dan n(A∪B∪C)
* Aplikasi himpunan dalam kehidupan

**Materi 4.3.6: Himpunan Bilangan**
* Himpunan bilangan asli (N)
* Himpunan bilangan cacah (W)
* Himpunan bilangan bulat (Z)
* Himpunan bilangan rasional (Q) dan irasional (I)
* Himpunan bilangan real (R)

---

## SECTION 5: Matriks dan Aplikasi Matematika

### Topic 5.1: Matriks

**Materi 5.1.1: Pengenalan Matriks** *(GRATIS)*
* Pengertian matriks dan notasinya
* Ordo matriks (baris × kolom)
* Elemen matriks
* Jenis-jenis matriks (baris, kolom, persegi, nol, identitas)
* Kesamaan dua matriks

**Materi 5.1.2: Operasi Matriks (Penjumlahan dan Pengurangan)**
* Syarat penjumlahan dan pengurangan matriks
* Cara menjumlahkan matriks
* Cara mengurangkan matriks
* Sifat-sifat penjumlahan matriks
* Matriks nol sebagai elemen identitas penjumlahan

**Materi 5.1.3: Perkalian Skalar dengan Matriks**
* Pengertian perkalian skalar
* Cara mengalikan skalar dengan matriks
* Sifat-sifat perkalian skalar
* Distribusi perkalian skalar terhadap penjumlahan
* Aplikasi perkalian skalar

**Materi 5.1.4: Perkalian Dua Matriks**
* Syarat perkalian dua matriks
* Cara mengalikan dua matriks
* Ordo hasil perkalian matriks
* Sifat-sifat perkalian matriks (asosiatif, distributif)
* Perkalian matriks tidak komutatif

**Materi 5.1.5: Transpose Matriks**
* Pengertian transpose matriks
* Notasi transpose: Aᵀ atau A'
* Sifat-sifat transpose
* Transpose hasil penjumlahan dan perkalian
* Matriks simetris

**Materi 5.1.6: Determinan Matriks**
* Pengertian determinan
* Determinan matriks ordo 2×2
* Determinan matriks ordo 3×3 (metode Sarrus)
* Sifat-sifat determinan
* Aplikasi determinan

**Materi 5.1.7: Invers Matriks**
* Pengertian matriks invers
* Syarat matriks memiliki invers (det ≠ 0)
* Rumus invers matriks 2×2
* Sifat-sifat invers matriks
* Penyelesaian sistem persamaan linear dengan invers matriks

### Topic 5.2: Pengukuran dan Konversi Satuan

**Materi 5.2.1: Pengenalan Satuan dan Pengukuran** *(GRATIS)*
* Pengertian satuan dan pengukuran
* Satuan baku dan tidak baku
* Sistem satuan internasional (SI)
* Jenis-jenis besaran (panjang, massa, waktu, dll)
* Pentingnya satuan dalam matematika

**Materi 5.2.2: Satuan Panjang**
* Tangga konversi satuan panjang (km, hm, dam, m, dm, cm, mm)
* Mengubah satuan panjang
* Operasi hitung dengan satuan panjang berbeda
* Satuan panjang tidak baku (inch, kaki, yard, mil)
* Aplikasi satuan panjang

**Materi 5.2.3: Satuan Massa (Berat)**
* Tangga konversi satuan massa (kg, hg, dag, g, dg, cg, mg)
* Mengubah satuan massa
* Satuan massa besar (ton, kuintal)
* Satuan massa tidak baku (ons, pon)
* Aplikasi satuan massa

**Materi 5.2.4: Satuan Waktu**
* Satuan waktu (detik, menit, jam, hari, minggu, bulan, tahun)
* Konversi satuan waktu
* Operasi hitung waktu
* Format waktu 12 jam dan 24 jam
* Aplikasi satuan waktu

**Materi 5.2.5: Satuan Luas**
* Hubungan satuan panjang dan luas
* Konversi satuan luas (km², m², cm², mm²)
* Satuan luas agraria (hektar, are)
* Mengubah satuan luas
* Aplikasi satuan luas

**Materi 5.2.6: Satuan Volume**
* Hubungan satuan panjang dan volume
* Konversi satuan volume (m³, dm³, cm³)
* Satuan volume untuk cairan (liter)
* Hubungan liter dengan m³ dan cm³
* Aplikasi satuan volume

### Topic 5.3: Perbandingan

**Materi 5.3.1: Pengenalan Perbandingan** *(GRATIS)*
* Pengertian perbandingan (rasio)
* Cara menyatakan perbandingan
* Menyederhanakan perbandingan
* Perbandingan dua besaran
* Perbandingan lebih dari dua besaran

**Materi 5.3.2: Perbandingan Senilai**
* Konsep perbandingan senilai
* Grafik perbandingan senilai
* Menyelesaikan masalah perbandingan senilai
* Aplikasi dalam kecepatan, harga, dan jarak
* Soal cerita perbandingan senilai

**Materi 5.3.3: Perbandingan Berbalik Nilai**
* Konsep perbandingan berbalik nilai
* Grafik perbandingan berbalik nilai
* Menyelesaikan masalah perbandingan berbalik nilai
* Aplikasi dalam pekerjaan dan waktu
* Soal cerita perbandingan berbalik nilai

**Materi 5.3.4: Skala**
* Pengertian skala
* Skala pada peta dan denah
* Menentukan jarak sebenarnya dari peta
* Menentukan jarak pada peta
* Aplikasi skala dalam kehidupan

**Materi 5.3.5: Perbandingan Bertingkat**
* Konsep perbandingan bertingkat
* Menyelesaikan perbandingan tiga besaran atau lebih
* Teknik penyelesaian dengan mencari nilai tengah
* Soal cerita perbandingan bertingkat
* Aplikasi dalam pembagian dan distribusi

**Materi 5.3.6: Aplikasi Perbandingan**
* Perbandingan dalam campuran (larutan, adonan)
* Perbandingan dalam pembagian (uang, barang)
* Perbandingan dalam resep masakan
* Perbandingan dalam model dan miniatur
* Soal cerita kompleks perbandingan

### Topic 5.4: Logika Matematika

**Materi 5.4.1: Pengenalan Logika Matematika** *(GRATIS)*
* Pengertian pernyataan (proposisi)
* Kalimat terbuka dan kalimat tertutup
* Nilai kebenaran (benar atau salah)
* Negasi (ingkaran) pernyataan
* Simbol-simbol dalam logika matematika

**Materi 5.4.2: Konjungsi dan Disjungsi**
* Pengertian konjungsi (dan/∧)
* Tabel kebenaran konjungsi
* Pengertian disjungsi (atau/∨)
* Tabel kebenaran disjungsi
* Aplikasi konjungsi dan disjungsi

**Materi 5.4.3: Implikasi**
* Pengertian implikasi (jika...maka/→)
* Tabel kebenaran implikasi
* Konvers, invers, dan kontraposisi
* Implikasi logis (ekuivalen)
* Aplikasi implikasi

**Materi 5.4.4: Biimplikasi**
* Pengertian biimplikasi (jika dan hanya jika/↔)
* Tabel kebenaran biimplikasi
* Hubungan implikasi dan biimplikasi
* Ekuivalensi logika
* Aplikasi biimplikasi

**Materi 5.4.5: Penarikan Kesimpulan**
* Modus ponens
* Modus tollens
* Silogisme
* Validitas argumen
* Aplikasi penarikan kesimpulan

**Materi 5.4.6: Ingkaran Pernyataan Majemuk**
* Ingkaran konjungsi (hukum De Morgan)
* Ingkaran disjungsi
* Ingkaran implikasi
* Ingkaran biimplikasi
* Ingkaran pernyataan berkuantor

## SECTION 6: Kalkulus

### Topic 6.1: Limit Fungsi

**Materi 6.1.1: Pengenalan Limit Fungsi** *(GRATIS)*
* Konsep limit dan intuisi grafis
* Notasi limit: lim f(x) saat x → a
* Limit kiri dan limit kanan
* Syarat eksistensi limit
* Perbedaan limit dan nilai fungsi

**Materi 6.1.2: Menghitung Limit Aljabar**
* Teknik substitusi langsung
* Teknik faktorisasi untuk bentuk tak tentu
* Teknik perkalian sekawan
* Sifat-sifat limit (penjumlahan, perkalian, pembagian)
* Limit fungsi polinomial dan rasional

**Materi 6.1.3: Limit Bentuk Tak Tentu**
* Bentuk tak tentu 0/0 dan karakteristiknya
* Bentuk tak tentu ∞/∞ dan karakteristiknya
* Teknik pembagian dengan pangkat tertinggi
* Limit fungsi trigonometri (lim sin x/x = 1)
* Strategi memilih teknik penyelesaian yang tepat

**Materi 6.1.4: Limit Tak Hingga dan Limit di Tak Hingga**
* Pengertian limit tak hingga (x → ∞)
* Perilaku fungsi saat x mendekati tak hingga
* Asimtot horizontal dan hubungannya dengan limit
* Limit fungsi rasional di tak hingga
* Limit fungsi irasional di tak hingga

---

### Topic 6.2: Turunan

**Materi 6.2.1: Pengenalan Turunan** *(GRATIS)*
* Konsep laju perubahan dan kemiringan
* Definisi turunan dari konsep limit
* Notasi turunan: f'(x), dy/dx, y'
* Turunan sebagai fungsi baru
* Interpretasi geometris turunan

**Materi 6.2.2: Aturan Turunan Dasar**
* Turunan konstanta dan fungsi pangkat
* Aturan perkalian konstanta
* Aturan penjumlahan dan pengurangan
* Turunan fungsi polinomial
* Turunan fungsi rasional sederhana

**Materi 6.2.3: Aturan Rantai, Perkalian, dan Pembagian**
* Aturan perkalian dua fungsi (product rule)
* Aturan pembagian dua fungsi (quotient rule)
* Aturan rantai (chain rule)
* Kombinasi aturan turunan
* Turunan fungsi komposisi

**Materi 6.2.4: Turunan Fungsi Trigonometri, Eksponen, dan Logaritma**
* Turunan sin x, cos x, tan x
* Turunan fungsi eksponen (eˣ, aˣ)
* Turunan fungsi logaritma (ln x, log x)
* Kombinasi dengan aturan rantai
* Turunan fungsi-fungsi khusus lainnya

**Materi 6.2.5: Turunan Tingkat Tinggi**
* Pengertian turunan kedua f''(x)
* Turunan ketiga dan seterusnya
* Interpretasi turunan kedua sebagai kecekungan fungsi
* Hubungan turunan pertama dan kedua dalam analisis fungsi
* Notasi turunan tingkat tinggi

---

### Topic 6.3: Integral

**Materi 6.3.1: Pengenalan Integral Tak Tentu** *(GRATIS)*
* Konsep integral sebagai anti-turunan
* Notasi integral: ∫f(x)dx
* Konstanta integrasi C dan maknanya
* Hubungan integral dan turunan
* Verifikasi hasil integral dengan turunan

**Materi 6.3.2: Aturan Integral Dasar**
* Integral konstanta dan fungsi pangkat
* Aturan penjumlahan dan pengurangan
* Integral fungsi polinomial
* Integral fungsi eksponen dan logaritma
* Integral fungsi trigonometri dasar

**Materi 6.3.3: Teknik Integral Substitusi**
* Konsep substitusi dalam integral
* Langkah-langkah metode substitusi
* Menentukan substitusi yang tepat
* Substitusi untuk fungsi komposisi
* Substitusi trigonometri sederhana

**Materi 6.3.4: Integral Tentu**
* Konsep integral tentu dan batas integrasinya
* Teorema dasar kalkulus
* Cara menghitung integral tentu
* Sifat-sifat integral tentu
* Hubungan integral tentu dengan konsep luas

---

### Topic 6.4: Optimasi dan Pengukuran

**Materi 6.4.1: Analisis Fungsi dengan Turunan** *(GRATIS)*
* Fungsi naik dan turun berdasarkan uji turunan pertama
* Titik kritis dan cara menentukannya
* Kecekungan fungsi berdasarkan turunan kedua
* Titik belok (inflection point)
* Sketsa grafik fungsi menggunakan informasi turunan

**Materi 6.4.2: Nilai Ekstrem**
* Nilai maksimum dan minimum lokal
* Uji turunan pertama untuk menentukan nilai ekstrem
* Uji turunan kedua untuk menentukan nilai ekstrem
* Nilai maksimum dan minimum global pada interval tertutup
* Perbedaan ekstrem lokal dan global

**Materi 6.4.3: Optimasi dalam Konteks Nyata**
* Pemodelan matematika untuk masalah optimasi
* Optimasi luas dan keliling bangun datar
* Optimasi volume dan luas permukaan bangun ruang
* Optimasi dalam konteks ekonomi (fungsi keuntungan dan biaya)
* Optimasi dalam konteks fisika (posisi, kecepatan, percepatan)

**Materi 6.4.4: Pengukuran Luas dengan Integral**
* Luas daerah di bawah kurva dan sumbu x
* Luas daerah antara dua kurva
* Menentukan batas integral dari kondisi yang diberikan
* Penanganan luas daerah dengan fungsi bernilai negatif
* Interpretasi geometris integral tentu

**Materi 6.4.5: Pengukuran Volume dengan Integral**
* Konsep volume benda putar
* Volume benda putar terhadap sumbu x (metode cakram)
* Volume benda putar terhadap sumbu y
* Metode cincin untuk benda putar dengan rongga
* Interpretasi geometris integral dalam tiga dimensi