// pages/panel/exam/exam-categories/index.tsx
...
const ExamCategoriesPage: React.FC = () => {
  ...
  const formatGrade = (grades: number[]) => {
    ...
    return (
      <div className="tw-flex tw-gap-1 tw-justify-center tw-flex-wrap">
        {grades.sort((a, b) => a - b).map((grade, index) => (
  ...
  const handleDelete = async (row: any) => {
    if (window.confirm(`Apakah Anda yakin ingin menghapus kategori ujian "${row.name}"?`)) {
      try {
        ...
        if (response.ok) {
          alert('Kategori ujian berhasil dihapus');
        ...
        alert('Terjadi kesalahan saat menghapus kategori ujian');
      }
    }
  };
  ...
  const reportConfig: ReportConfig = {
    title: 'Manajemen Kategori Ujian (Exam Categories)',
    columns,
    colGroups: [
      {
        key: 'basic',
  ...
  return (
    <MainLayout>
      <div className="tw-min-h-screen tw-bg-gradient-to-br tw-from-blue-50 tw-via-white tw-to-blue-50">
        <ReportLayout
          config={reportConfig}
          apiEndpoint="/exam-types/paged?kind=0"
          fetchOnMount={true}
          searchMode="server"
